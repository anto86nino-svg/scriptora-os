...
const KEYWORD_GOLD_SCHEMA = {
  ...
  properties: {
    ...
    backendKeywords: { type: "array", items: { type: "string" } },
    goldKeywords: {
      type: "array",
      items: {
        type: "object",
        properties: {
          keyword: { type: "string" },
          intent: { type: "string", enum: ["buyer", "problem", "genre", "audience", "benefit", "competitor-gap"] },
          strength: { type: "number" },
          competitionRisk: { type: "string", enum: ["low", "medium", "high"] },
          why: { type: "string" },
        },
        required: ["keyword", "intent", "strength", "competitionRisk", "why"],
      },
    },
    ...
    positioning: {
      type: "object",
      properties: {
        mainAudience: { type: "string" },
        commercialPromise: { type: "string" },
        strongestAngle: { type: "string" },
        saturationWarning: { type: "string" },
      },
      required: ["mainAudience", "commercialPromise", "strongestAngle", "saturationWarning"],
    },
...
async function handleKeywordGold(p: any) {
  const title = String(p?.title || "").trim();
  const subtitle = String(p?.subtitle || "").trim();
  const genre = String(p?.genre || "books").trim();
  const language = String(p?.language || "Italian").trim();
  const marketplace = String(p?.marketplace || "amazon.it").trim();

  if (!title) throw new Error("Title is required for Keyword Gold");
  ...
  const queries = [
    `${site} ${title} ${subtitle} libri`,
    `${site} ${genre} bestseller libri ${title}`,
    `${site} ${genre} kindle bestseller keywords`,
    `${genre} BISAC categories books ${title}`,
    `${title} ${subtitle} amazon book reviews readers`,
  ].filter(q => q.trim().length > 8);

  const { results, text, usedQueries } = await braveMultiQuery(queries);

  const sys = `You are a senior Amazon KDP metadata strategist and BISAC/category specialist.
You create honest, publish-ready metadata from title/subtitle and real public market signals.
Do NOT claim official Amazon data. Use public signals only.
Return STRICT JSON only.`;
  ...
  const usr =
    ...
    `- Language: ${language}\n` +
    `- Marketplace: ${marketplace}\n\n` +
    (text ? `Public market signals from Brave Search:\n${text}\n\n` : `No live results available. Use careful category reasoning.\n\n`) +
    `Generate Keyword Gold metadata:
- 3-5 BISAC-style category paths.
- 3-5 Amazon/KDP browse category suggestions.
- exactly 7 backendKeywords, KDP-safe, no repeated words, no author names, no misleading claims.
- 10-15 goldKeywords with intent, strength 0-100, competitionRisk, and why.
- negativeKeywords to avoid.
- positioning: mainAudience, commercialPromise, strongestAngle, saturationWarning.
- finalChecklist: 5 practical actions before publishing.
Write all explanations in ${language}.`;

  const out = await callAIJson(sys, usr, "keyword_gold", KEYWORD_GOLD_SCHEMA);
...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      ...
      case "keywordGold":           result = await handleKeywordGold(payload); break;
