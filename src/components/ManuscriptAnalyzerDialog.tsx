...
function detectGenre(text: string): Genre {
  ...
  if (dialogueMarks > 20 || /\b(prologue|capitolo|chapter)\b/i.test(text)) return "memoir";
  if (/\b(marketing|business|sales|company|cliente|azienda|vendite|mercato)\b/.test(lower)) return "business";
  if (/\b(manuale|tutorial|procedura|installazione|step|passo|software)\b/.test(lower)) return "manual";
  if (/\b(amore|heart|love|relationship|romance)\b/.test(lower)) return "romance";
  if (/\b(murder|killer|detective|omicidio|indagine|suspense)\b/.test(lower)) return "thriller";
  return "self-help";
}
...
function analyzeChapter(title: string, content: string): ManuscriptChapterAnalysis {
...
function summaryKeyForScore(score: number): string {
  if (score >= 86) return "manuscript_summary_strong";
  if (score >= 74) return "manuscript_summary_good";
  if (score >= 62) return "manuscript_summary_needs_work";
  return "manuscript_summary_fragile";
}

function analyzeManuscript(text: string, title: string, sourceName: string): ManuscriptAnalysis {
  const chapters = splitIntoChapters(text).map(chapter => analyzeChapter(chapter.title, chapter.content));
...
export function ManuscriptAnalyzerDialog({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      const detectedGenre = detectGenre(text);
      setRawText(text);
