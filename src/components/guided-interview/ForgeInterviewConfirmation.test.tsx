...
describe("ForgeInterviewConfirmation", () => {
  it("shows editorial summary without technical labels", () => {
    ...
    expect(screen.getByText(/Il libro che ho capito/i)).toBeInTheDocument();
  ...
  it("fires confirm when ready", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /Blocca DNA/i }));
  ...
  it("does not show ready title when blueprint is not ready", () => {
    ...
    expect(screen.queryByText(/Il libro che ho capito/i)).not.toBeInTheDocument();
