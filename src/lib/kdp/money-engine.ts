...
function buildTitleDominationFallback(input: DominateTitlesInput): TitleDominationResult {
  ...
  type TitleFallbackTemplate = [
    title: string,
    subtitle: string,
    score: number,
    risk: "low" | "medium" | "high",
  ];
  const templates: TitleFallbackTemplate[] = italian
    ? [
...
function buildTrendingNichesFallback(input: TrendingNichesInput): TrendingNichesResult {
  ...
  const marketplaces: TrendingNiche["marketplace"][] = input.marketplaces?.length
    ? input.marketplaces
  ...
  const baseIdeas: TrendingNicheSeed[] = italian
    ? [
  ...
  const niches: TrendingNiche[] = baseIdeas.map(([name, targetReader, dominantPromise, suggestedAngle, keywords], index) => ({
    ...
    targetReader,
    suggestedAngle,
    dominantKeywords: [topic, ...keywords].slice(0, 6),
    whyItMatters: italian
